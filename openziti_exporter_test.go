...
var (
	binary = filepath.Join(os.Getenv("PWD"), "openziti_exporter")
)

const (
	address = "localhost:19184"
)

func TestFileDescriptorLeak(t *testing.T) {
	if _, err := os.Stat(binary); err != nil {
		t.Skipf("openziti_exporter binary not available, try to run `make build` first: %s", err)
	}
...
func queryExporter(address string) error {
	queryReq, err := http.NewRequestWithContext(context.Background(), http.MethodGet,
		fmt.Sprintf("http://%s/metrics", address), http.NoBody)
	if err != nil {
		return err
	}
